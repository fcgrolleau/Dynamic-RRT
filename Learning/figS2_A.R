temp <- merge(data.frame(id=actual_ids$ids_k1, r1=ite_k1 > 0, a1=actual_ttts$a1),
      data.frame(id=actual_ids$ids_k2, r2=ite_k2>0), by="id", all.x = TRUE)

temp <- merge(temp, data.frame(id=actual_ids$ids_k2, a2=actual_ttts$a2), by="id", all.x = TRUE)

temp <- merge(temp, data.frame(id=actual_ids$ids_k3, r3=ite_k3 > 0, a3=actual_ttts$a3), by="id", all.x = TRUE)
head(temp)
...
library(htmlwidgets)
#htmlwidgets::saveWidget(widget = pl, file="sankey.html")
