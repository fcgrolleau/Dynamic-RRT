temp <- merge(data.frame(id=actual_ids_val$ids_k1, r1=ite_k1_val - qnorm(.975) * ite_se_k1_val > 0, a1=actual_ttts_val$a1),
              data.frame(id=actual_ids_val$ids_k2, r2=ite_k2_val - qnorm(.975) * ite_se_k2_val > 0), by="id", all.x = TRUE)
temp$r2[temp$id %in% phi_2_ids] <- 777

temp <- merge(temp, data.frame(id=actual_ids_val$ids_k2, a2=actual_ttts_val$a2), by="id", all.x = TRUE)
temp$a2[temp$id %in% phi_2_ids] <- 777

temp <- merge(temp, data.frame(id=actual_ids_val$ids_k3, r3=ite_k3_val - qnorm(.975) * ite_se_k3_val > 0, a3=actual_ttts_val$a3), by="id", all.x = TRUE)
temp$r3[temp$id %in% phi_3_ids] <- 777
temp$a3[temp$id %in% phi_3_ids] <- 777
head(temp, 30)
...
temp2 <- as.data.frame(lapply(temp2, function(x) str_replace(x, '_777', ': D') ))
...
pl <- highchart() %>%
  hc_add_series(data = data_to_sankey(sank), type = "sankey"
                ...
                ,   nodes = list(list(id =  names(table(sank$New_r1))[1] , color = "gray" )
                                 ...
                                 ,list(id = names(table(sank$New_r2))[4] , color = "gray" )
                                 ,list(id = names(table(sank$New_a2))[1] , color = "gray" )
                                 ,list(id = names(table(sank$New_a2))[2] , color = "gray" )
                                 ,list(id = names(table(sank$New_a2))[3] , color = "gray" )
                                 ,list(id = names(table(sank$New_a2))[4] , color = "gray" )
                                 ...
                                 ,list(id = names(table(sank$New_r3))[4] , color = "gray" )
                                 ,list(id = names(table(sank$New_a3))[1] , color = "gray" )
                                 ,list(id = names(table(sank$New_a3))[2] , color = "gray" )
                                 ,list(id = names(table(sank$New_a3))[3] , color = "gray" ) 
                )) %>%
...
library(htmlwidgets)
#htmlwidgets::saveWidget(widget = pl, file="sankey_val_s.html")
